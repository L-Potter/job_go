...
func updateUserHandler(c *gin.Context) {
	...
	var existingID int
	var existingEmployeeID, existingName, existingRole string
	var existingShiftType, existingSite, existingDayNight, existingGroup sql.NullString
	var existingCap sql.NullInt64
	err := mainDB.QueryRow(
		`SELECT user_id, employee_id, name, shift_type, site, day_night, role, "group", monthly_overtime_cap_hours FROM users WHERE user_id = ?`, userID,
	).Scan(&existingID, &existingEmployeeID, &existingName, &existingShiftType, &existingSite, &existingDayNight, &existingRole, &existingGroup, &existingCap)
	if err == sql.ErrNoRows {
		c.JSON(http.StatusNotFound, gin.H{"error": "用户不存在"})
		return
	} else if err != nil {
	...
	if rowsAffected > 0 {
		...
		getOldStr := func(ns sql.NullString) string {
		...
		if req.Name != "" && req.Name != existingName {
			changed = append(changed, fmt.Sprintf("姓名:%s→%s", existingName, req.Name))
		}
		if req.EmployeeID != "" && req.EmployeeID != existingEmployeeID {
			changed = append(changed, fmt.Sprintf("工號:%s→%s", existingEmployeeID, req.EmployeeID))
		}
		if req.ShiftType != nil && *req.ShiftType != existingShiftType.String {
			changed = append(changed, fmt.Sprintf("shift_type:%s→%s", getOldStr(existingShiftType), *req.ShiftType))
		}
		if req.Site != nil && *req.Site != existingSite.String {
			changed = append(changed, fmt.Sprintf("site:%s→%s", getOldStr(existingSite), *req.Site))
		}
		if req.DayNight != nil && *req.DayNight != existingDayNight.String {
			changed = append(changed, fmt.Sprintf("day_night:%s→%s", getOldStr(existingDayNight), *req.DayNight))
		}
		if req.Role != "" && req.Role != existingRole {
			changed = append(changed, fmt.Sprintf("role:%s→%s", existingRole, req.Role))
		}
		if req.Group != nil && *req.Group != existingGroup.String {
			changed = append(changed, fmt.Sprintf("group:%s→%s", getOldStr(existingGroup), *req.Group))
		}
		
		oldCapStr := "(預設)"
		if existingCap.Valid {
			oldCapStr = fmt.Sprintf("%d", existingCap.Int64)
		}

		if req.ClearMonthlyOvertimeCap != nil && *req.ClearMonthlyOvertimeCap {
			if existingCap.Valid {
				changed = append(changed, fmt.Sprintf("monthly_overtime_cap_hours:%s→(恢復預設)", oldCapStr))
			}
		} else if req.MonthlyOvertimeCapHours != nil {
			if !existingCap.Valid || existingCap.Int64 != int64(*req.MonthlyOvertimeCapHours) {
				changed = append(changed, fmt.Sprintf("monthly_overtime_cap_hours:%s→%d", oldCapStr, *req.MonthlyOvertimeCapHours))
			}
		}
...
func setCalendarTagHandler(c *gin.Context) {
	...
	if err == sql.ErrNoRows {
		...
		logAdminActionWithActor(
			...
			"calendar_tags",
			date,
			fmt.Sprintf("建立行事曆標記（日期 %s，假日:%v，班別:%v，註解:%s）", date, isHoliday, req.ShiftType, req.Comment),
		)
	} else if err != nil {
		...
		logAdminActionWithActor(
			...
			"calendar_tags",
			date,
			fmt.Sprintf("更新行事曆標記（日期 %s，假日:%s，班別:%s，註解:%s）", date, holidayDesc, shiftVal, req.Comment),
		)
	}
}
...
func deleteCalendarTagHandler(c *gin.Context) {
	...
	logAdminActionWithActor(
		...
		"calendar_tags",
		date,
		fmt.Sprintf("刪除行事曆標記（日期 %s，註解:%s）", date, req.Comment),
	)
	...
	if len(errors) > 0 {
		...
		if successCount > 0 {
			logAdminActionWithActor(
				...
				"calendar_tags",
				"batch",
				fmt.Sprintf("批次更新行事曆：成功 %d 筆／共 %d 筆，內容:%+v", successCount, len(req.Tags), req.Tags),
			)
		}
	} else {
		c.JSON(http.StatusOK, gin.H{
		...
		logAdminActionWithActor(
			...
			"calendar_tags",
			"batch",
			fmt.Sprintf("批次更新行事曆：全部成功（共 %d 筆），內容:%+v", successCount, req.Tags),
		)
	}
}
